...
function createAuth(opts)
{
   ...
   if (!Array.isArray(_roles))
      throw new Error("invalid parameter: opts.roles must be an array");

   for (let role of _roles)
   {
      if (typeof(role) !== "string")
         throw new Error("invalid parameter: opts.roles must be an array of strings");
   }

         // validate the users -> if a user has a role assignment, it must be a valid role
   if (!_users)
      throw new Error("invalid parameter: opts.users - not found");

   if (!Array.isArray(_users))
      throw new Error("invalid parameter: opts.users is not an array");

   for (let user of _users)
   {
      if (typeof(user) !== "object")
         throw new Error("invalid parameter: opts.users must be an array of objects");

      if (!user.user || !user.pwd)
         throw new Error("invalid parameter: user object must have properties: user, pwd");

      if (user.roles)
      {
         if (!Array.isArray(user.roles))
            throw new Error("invalid parameter: opts.users[].roles is not an array");

         for (let role of user.roles)
         {
            if (typeof(role) !== "string")
               throw new Error("invalid parameter: opts.users[].role not a string");

            if (!_roles.includes(role))
               throw new Error("invalid parameter: invalid users[].role value");
         }
      }
   }
   ...
   function createRoleVerifier(validRoles)
   {
      ...
      if (!Array.isArray(validRoles))
         throw new Error("invalid parameter: validRoles must be an array");

      if (0 === validRoles.length)
         throw new Error("invalid parameter: no roles specified");

      for (let role of validRoles)
      {
         if (typeof(role) !== "string")
            throw new Error("invalid parameter: validRoles must be an array of strings");

         if (!_roles.includes(role))
            throw new Error("invalid parameter: unknown role value");
      }
