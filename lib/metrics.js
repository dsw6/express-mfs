...
const debug = require("debug")("express-mfs:metrics");
const os = require("os");
...
const rpsStats = {
   ...
   // extra info functions, used by the metrics middleware
const cbInfoFuncs = [];
...
/**
 * Initialize the collection of metrics.
 * @param {Object}  opts              - opts for the metrics module
 * @param {boolean}    opts.totals    - optional, collect request totals (total requests, etc)
 * @param {boolean}    opts.rps       - optional, collect req/sec metrics 
 * @param {boolean}    opts.methods   - optional, collect method metrics
 ...
 * called, in order, and their response data will be added to the metrics response.  An info function
 * has no parameters and must return an object with two properties:  "name" and "value".  The
 * json data returned from metrics will include an additional property ("name") with the value.
 *
...
function init(opts)
{
   ...
   debug(`init: ${util.format(opts)}`);
      ...
      // reset all metrics configuration each time init is called
   resetStats();
   ...
   if (opts.hasOwnProperty("totals"))
   {
      if (typeof(opts.totals) !== "boolean")
         throw new Error("express-mfs: invalid parameter, opts.totals must be boolean");      
   ...
   if (opts.hasOwnProperty("rps"))
   {
      if (typeof(opts.rps) !== "boolean")
         throw new Error("express-mfs: invalid parameter, opts.rps must be boolean");      
   ...
   if (opts.hasOwnProperty("methods"))
   {
      if (typeof(opts.methods) !== "boolean")
         throw new Error("express-mfs: invalid parameter, opts.methods must be boolean");      
   ...
   if (opts.hasOwnProperty("methodInfo"))
   {
      if (typeof(opts.methodInfo) !== "function")
         throw new Error("express-mfs: invalid parameter: opts.methodInfo is not a function");
   ...
   if (opts.hasOwnProperty("extraInfo"))
   {
      if (!Array.isArray(opts.extraInfo))
         throw new Error("express-mfs: invalid parameter: opts.extraInfo is not an array");

      for (let func of opts.extraInfo)
      {
         if (typeof(func) !== "function")
            throw new Error("express-mfs: invalid parameter: invalid extraInfo is not a function");
...
function updateRPS()
{
   ...
   // ----------- metrics Callback Func ---------------------------------------------
   //    This function is registered by the collect middleware to collect metrics
...
function updateStats()
{
   ...
   if (reqStats.enabled)
   {
      ...
      if ( statusCode >= 200 && statusCode < 300 )
         ...
         // collect the rps level metrics, increment the current sample count
   if (rpsStats.enabled)
      rpsStats.current.count++;

         // collect the method level metrics
   if (req.mfsMetrics)
   {
...
function name(name)
{
   if (typeof(name) !== "string")
      throw new Error("express-mfs: invalid parameter:name must be a string");
   ...
   // ----------- metrics collection ----------------------------------------------
   //  Support to manually set the method name inside middleware functions
